...
const DashboardPage: React.FC = () => {
  const { user } = useAuth();
  const { indices, topStocks, isMarketOpen, refreshData, loading, error } = useData();
  if (loading) {
  return <DashboardLayout><div>Loading market data...</div></DashboardLayout>;
}
...
<div className="mb-8">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    {quickActions.map((action) => { // You don't need the index anymore
      ...
      return (
        <div
          key={action.title} // <--- THIS IS THE FIX
              ...
              {indices.map((index) => (
                <div key={index.symbol || index.name} className="text-center">
                  ...
                  <div className={`flex items-center justify-center space-x-1 text-sm ${
                    ...
                    {typeof index.change === 'number' && index.change >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                    <span>
